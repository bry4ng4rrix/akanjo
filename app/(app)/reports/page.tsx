...
import { Download, TrendingUp, TrendingDown, Package, AlertTriangle, Loader2 } from 'lucide-react';
...
function ReportsContent() {
  ...
  const loadReportData = async () => {
    try {
      ...
      formattedStats.forEach((stat: ProductStat) => {
        sizeMap.set(stat.size, (sizeMap.get(stat.size) || 0) + stat.movement_count);
      });

      const totalMovements = Array.from(sizeMap.values()).reduce((a, b) => a + b, 0);
      const sizeData = Array.from(sizeMap.entries()).map(([size, count]) => ({
        size,
        count,
        percentage: totalMovements > 0 ? Math.round((count / totalMovements) * 100) : 0,
      }));

      setSizeDistribution(sizeData.sort((a, b) => {
        const order = { S: 0, M: 1, XL: 2, XXL: 3 };
        return (order[a.size as keyof typeof order] || 999) - (order[b.size as keyof typeof order] || 999);
      }));
    } catch (error) {
  ...
  const exportToExcel = async () => {
    try {
      ...
      const productData = stats.map((stat) => ({
        ...
        'Dernier mouvement': stat.last_movement_at ? new Date(stat.last_movement_at).toLocaleDateString() : 'N/A',
      }));
      ...
      // Download
      XLSX.writeFile(workbook, `rapport-vetements-${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Analyse complète de votre stock de vêtements</p>
        </div>
        <Button onClick={exportToExcel} disabled={exportLoading} className="gap-2">
          {exportLoading ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{new Set(stats.map((s) => s.product_id)).size}</div>
          </CardContent>
        </Card>
      </div>
                ...
                {slowMovingProducts.map((product) => (
                  <div key={product.id} className="flex items-center justify-between p-3 bg-orange-50 rounded">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Taille {product.size}</p>
                    </div>
                    <Badge variant="outline">{product.movement_count} mouvements</Badge>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-slate-500 text-center py-4">Aucun produit lent</p>
            )}
          </CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {stats.map((stat) => (
                  <TableRow key={stat.id}>
                    <TableCell className="font-medium">{stat.product?.name || 'N/A'}</TableCell>
                    <TableCell className="font-mono text-sm">{stat.product?.sku}</TableCell>
                    <TableCell>
                      <Badge variant="outline">{stat.size}</Badge>
                    </TableCell>
                    <TableCell className="text-right">{stat.movement_count}</TableCell>
                    <TableCell className="text-right">
                      <span className="text-red-500 font-semibold">{stat.total_sold}</span>
                    </TableCell>
                    <TableCell className="text-right">
                      <span className="text-green-500 font-semibold">{stat.total_purchased}</span>
                    </TableCell>
